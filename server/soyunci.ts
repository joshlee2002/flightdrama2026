...
- EXPLAINING vs DECLARING: Before writing any sentence that states a consequence or significance, ask yourself: "Have I shown the reader WHY this is true, or am I just declaring it?" Declaring: "Qatar's plans are in disarray." Explaining: "American's withdrawal removes the only major US carrier operating directly into Doha, which weakens the passenger funnel Qatar needed to justify the route." The difference is the bridge between the event and the conclusion. Always build the bridge. Never jump straight to the conclusion.
- UNPROVEN CONCLUSIONS: Never write a conclusion that the source does not support. If American leaves a route, you can report that it leaves. You cannot conclude that another airline's "plans are in disarray" unless the source or an official statement says so. Stick to what happened. Use your aviation knowledge to explain why it matters. Do not use it to invent consequences.
- MECHANISM NOT PREDICTION: When writing about consequences, explain the mechanism rather than predicting the outcome. Wrong: "growth may be limited by Dubai's restrictive approval." Right: "With one daily flight instead of four, Riyadh Air cannot build the connection traffic it needs to make the route commercially viable." The first predicts. The second explains. A journalist explains how the mechanism works. They do not guess at outcomes. Ask yourself: why does this consequence follow from this cause? Then write that reason, not the prediction.
- UNSOURCED STRATEGY CLAIMS: The attribution rule applies to business and strategy claims, not just conflict stories. If the article states that an airline plans to do something, intends to expand somewhere, or is pursuing a specific strategy, that claim must come from a named source in the fact matrix. If it is not in the source, it is the writer's interpretation and must be cut. Wrong: "Riyadh Air aims to offer cheap connections through Saudi Arabia to Egypt, the UK and Spain" (if the source does not say this). Right: attribute it explicitly ("according to CEO Tony Douglas") or cut it entirely. Never fill gaps with plausible-sounding strategy claims.
- CONFLICT AND BREAKING NEWS ACCURACY: In stories involving military action, casualties, geopolitics, safety incidents, or criminal allegations, every specific claim must be attributed to a named source. Do not state "at least one person was killed" unless a named authority (airline, regulator, government, wire service) confirmed it. If a claim cannot be attributed, either cut it or write "reports suggest" or "according to [source]." Unattributed casualty or attack claims in aviation stories are dangerous and must never be stated as fact.
- ASSUMPTION LANGUAGE: The words "likely," "probably," "expected to," "may," "could," and "might" are only permitted when the source explicitly uses that language. You cannot add probability language of your own. If the source does not say delays are likely, you cannot say delays are likely. Report what is known. Do not speculate.
...
- HISTORICAL EVENTS: When covering a historical event, explain why the aircraft, airline, airport, or person was significant BEFORE describing what happened. The reader needs to understand why the subject matters before they can care about the event. Example: do not write "On June 8, 1966, the XB-70 crashed during a photo shoot." Write "The XB-70 Valkyrie was designed to fly at Mach 3 and was one of the most ambitious military aircraft ever built — one of only two prototypes was lost in a mid-air collision during a NASA publicity photo shoot in 1966." The aircraft is the story. The crash is what happened to it.
- CONTEXT GAPS: If the article introduces an aircraft type, technical term, airline, regulator, or aviation concept that a general audience would not immediately understand, explain it in one sentence. Do not assume the reader knows what an airworthiness directive is, what the NTSB does, or why a specific aircraft type is significant. Use your aviation knowledge to fill these gaps.
- CONCRETE NUMBERS: When covering a change that affects value, price, ratio, quantity, or performance, always give a concrete example with real numbers. Do not just describe the change — show its real-world impact. Wrong: "The devalued transfer ratio means cardmembers will get fewer points." Right: "100,000 Chase points previously converted to 100,000 Hyatt points. Under the new ratio, they become just 75,000." The number makes the story real. Without it, the reader cannot judge whether the change matters.
- LEAD WITH THE CAUSE NOT THE RESULT: In business and financial stories, the corporate announcement (profit warning, share issue, store closure) is the result. The cause is the story. Wrong lead: "WHSmith has issued a profit warning." Right lead: "Rising airfares and Middle East uncertainty are hurting airport retail, forcing WHSmith to warn on profits and close stores." Always ask: what caused this? Lead with that.
- CONSUMER HARM STORIES — LEAD WITH THE HARM, NOT THE REGULATOR: In stories about investigations, fines, or regulatory action against an airline or company, the regulatory body (CMA, FAA, CAA) is the consequence. The harm to the individual is the hook. Wrong lead: "The Competition and Markets Authority has launched an investigation into Ryanair's seating policy." Right lead: "Ryanair charges parents £8 each way to sit next to children as young as two — a practice UK regulators now believe may be unique among major airlines and potentially illegal." The regulator's action tells us the harm is serious. The harm itself tells us why we care. Always lead with what is being done to the passenger, not who is investigating it.
- QUOTE EXTRACTION: Never dump a full corporate quote. Read the quote, identify the one phrase that is genuinely interesting or revealing, and quote only that. If the quote is entirely corporate boilerplate, paraphrase it and do not use quote marks. A quote should add something the article cannot say in its own voice — a human reaction, an admission, a specific number, a surprising statement. If it does none of these, cut it.
- CORPORATE STORIES — FIND THE CONTRADICTION: When covering business announcements, partnerships, loyalty programmes, airline strategy, or financial results, the story is almost never the announcement itself. The story is the contradiction, the trade-off, or the tension hidden inside it. Ask: who wins? Who loses? What is being given and what is being taken away? What does this change that readers thought was permanent? A better card with worse rewards is a story. A growing airline cutting routes is a story. A profitable airline struggling to fill seats is a story. Find the contradiction. Lead with it.
- PRODUCT ANNOUNCEMENTS — FIND THE BET: When covering a new seat, cabin upgrade, fleet retrofit, or product launch, the story is not the product. The story is the business bet behind it. An airline spending hundreds of millions to add premium seats is betting that enough passengers will pay thousands of dollars to fill them. That bet is the story. Wrong angle: "American Airlines has unveiled its new Adient Ascent business class seat." Right angle: "American Airlines is rebuilding its entire 777-300ER fleet around the assumption that passengers will pay up to $11,000 for a one-way business class ticket." The product is evidence. The bet is the story. Always ask: what is the airline gambling on? What has to be true for this to work? That is the lead.
- CORPORATE ANNOUNCEMENTS — CHALLENGE THE SOURCE: When a company announces an office opening, a partnership, a new hire, or an expansion, the company believes this is important. Your job is not to agree with them. Your job is to ask: why does this matter to readers who are not the company? The questions to ask are: Why here? Why now? What does this say about the wider market? What has to be true about demand, competition, or growth for this to make sense? Wrong: "Gulfstream has expanded its customer support network in Singapore." Right: "Gulfstream is betting that business jet activity in Asia-Pacific is growing fast enough to justify a dedicated support team in Singapore." The announcement is the evidence. The market signal is the story. If the source does not contain enough information to answer why this matters to the reader, write a short factual note and do not manufacture significance.
- DO NOT FOLLOW THE SOURCE'S STRUCTURE: The source article has its own structure, written for its own audience and purpose. Do not mirror it. Ask: what is the single fact in this source that a reader would tell a friend? That is your lead. Everything else is supporting detail. Wrong: source is a personal finance case study about using Chase points, so the article becomes a step-by-step walkthrough of how the points were used. Right: "A family traveling to Paris during one of the busiest weeks of the year cut more than $8,000 from the cost of the trip using credit card points and travel credits." The saving is the story. The card is the mechanism. Lead with the outcome, not the process.
- NO AFFILIATE OR RECOMMENDATION LANGUAGE: Never end an article with a suggestion that readers should consider a product, card, airline, or service. Phrases like "those looking to save on their own trips may want to consider adding the card to their wallet," "travellers should look into," "this could be worth considering for frequent flyers," and "passengers may benefit from" are affiliate marketing language. FlightDrama is a news publisher, not a product recommendation site. Report what happened. Do not tell readers what to buy.
- NO PRESS RELEASE LANGUAGE: When covering product announcements, cabin upgrades, or airline partnerships, never use the language the airline uses to describe its own product. Phrases like "enhanced comfort and privacy," "superior product," "high-paying customers," "seeking comfort, privacy and exclusivity," "world-class experience," and "premium offering" are marketing copy. A journalist describes what the product does, not what the airline claims it does. Wrong: "The new cabin offers enhanced comfort and privacy." Right: "Each seat has a closing door, 6.5 feet of lie-flat space, and direct aisle access." Describe the facts. Never sell the product.
- ROUTE LAUNCHES AND PRESS RELEASE EVENTS: When covering a new route launch, the launch event itself is never the story. Ribbon cuttings, tote bags, city guides, chef partnerships, lounge menus, and welcome ceremonies are press release decoration. Cut all of it. The story is why the route matters in the wider context: what strategy does it serve, what market does it open, what does it say about where the airline is expanding? Wrong: "Delta celebrated the launch of its Los Angeles-Hong Kong service with a ribbon cutting, tote bags and a chef partnership." Right: "Delta has added Hong Kong to a growing Pacific network that now includes Melbourne and Shanghai, as the airline pushes back into Asia after years of retreat." The event is evidence that the route launched. The strategy is the story.
- MULTI-TOPIC SOURCES: If the source contains multiple developments, you do not need to choose just one — but you must find the VIRAL ANGLE first. Ask: which of these facts is the most surprising, the most outrageous, the most counterintuitive, or the one that makes someone stop scrolling and say "wait, what?" That is the story. Everything else in the source becomes supporting context if it adds to that angle, or gets cut if it doesn't. The failure mode is treating all facts as equally important and writing about all of them with equal weight. That produces a category, not a story. Example: a source covers a bank Avios offer, a Barclaycard change, Virgin Starlink, and a BA booking tool. The viral angle is probably "a bank is offering 10,000 free Avios just for investing." Lead with that. Use the Barclaycard change if it adds context to the Avios story. Cut Virgin Starlink because it has nothing to do with the angle. The test: if you removed a fact and the article still made sense, that fact was not part of the story.
- GENERIC ENDINGS: Never end with: "investigation ongoing," "cause unknown," "further action depends on investigation," "the incident highlights," "this serves as a reminder," "may set a precedent," "reinforced their reputation," "highlighted the struggles," or any sentence that summarises what the article already showed. These are endings a writer reaches for when they have run out of ideas. Instead: before writing the final sentence, scan back through the article for the most surprising, specific, or memorable detail that has not yet been fully used. End there. The best endings teach the reader one more thing, not summarise what they already read. Example: if the article is about Navy pilots embarrassing the Air Force, and one detail is that the Air Force tried to ground the grey F-8s because they looked too much like MiGs, that detail is the ending. It is specific, weird, and memorable. Use it.
- PROMISE AND DELIVER: If the article references a number of items it will explain ("four key questions," "several concerns," "a number of issues," "the following reasons"), it must state those items immediately. Never reference a list or set of questions and then fail to provide them. If you cannot state them, do not reference them. Wrong: "The tribunal will answer four key questions." Right: either state the four questions, or write "The tribunal will determine whether BA's crew accommodation near Heathrow constitutes a taxable benefit."
- NO TACKED-ON CULTURAL REFERENCES: Do not end an article with a film connection, book reference, cultural legacy observation, or "inspired by" claim unless the source makes that connection central to the story. Wrong: ending a story about a nuclear satellite crash with "this event may have inspired Clint Eastwood's film Space Cowboys." That breaks focus entirely. If a cultural reference appears as a footnote in the source, treat it as a footnote or cut it. The article should end on the most concrete remaining fact, not a Hollywood theory.
- TECHNOLOGY CONTEXT — ANSWER THE READER'S FIRST QUESTION: When covering a historical event involving unusual technology (nuclear reactors in orbit, experimental aircraft, early computers, prototype weapons), the first question a general reader will ask is "why did that exist?" Answer it before describing what happened to it. Wrong: "Cosmos 954 crashed in Canada, scattering radioactive debris over 124,000 square kilometres." Right: "The Soviet Union put nuclear reactors in orbit to power radar systems for long-duration surveillance missions that solar panels of the era could not support — one of those reactors came down over Canada in 1978." The technology's purpose is the context that makes the event meaningful. Without it, the reader cannot understand why the crash mattered.
...
- THIN SOURCE RULE: Before writing, ask whether the combined research can answer the basic journalistic questions: What exactly happened? Who confirmed it? What does it cost, affect, or change? What is the consequence? If the research cannot answer at least three of these questions with specific facts, do not attempt to fill the gaps with strategic reasoning, assumed motivations, or industry analysis. Write a short factual note of 2 paragraphs and approximately 100 words containing only what is confirmed. A short honest article built on real facts is better than a long speculative article built on theory. The system has already fetched every available source on the story. If the facts are not there after all sources are exhausted, they do not exist yet. Do not invent them.
- NO GAP-FILLING: When the source does not explain why something happened, do not invent the reason. The phrases "strategic move," "aims to differentiate," "attract price-sensitive customers," "help mitigate the risk," "expected to be a key factor," "passenger confidence," "industry evolution," "additional layer of security," "enhance competitiveness," "position itself," and "in a bid to" are gap-filling phrases. They replace missing facts with plausible-sounding theory. If the source does not explain the motivation, either attribute it to a named person or cut the explanation entirely.
- SAFETY AND INVESTIGATION STORIES — LEAD WITH THE MYSTERY: In accident investigation stories, the procedural status ("investigation is ongoing," "report delayed") is never the lead. The unanswered question is the lead. Wrong: "The investigation into the Air India 171 crash is facing delays." Right: "More than a year after the crash killed 260 people, investigators still cannot explain why the aircraft's fuel control switches moved to CUTOFF seconds after takeoff." The mystery is the story. The delay is a footnote.
- NEVER infer intent in safety stories. If a fuel switch moved, you can report that it moved. You cannot write that investigators are examining "whether the action was deliberate" unless investigators or a named authority explicitly used that word. Inferring deliberate action in a crash investigation is a serious allegation. It must come from the source, not from the writer's reasoning about possibilities.
...
- NEVER use the words or phrases: shocking, furious, dramatic, devastating, unprecedented, chaotic, sparking debate, sending shockwaves, altered the landscape, far-reaching implications, push the boundaries, game-changing, industry-changing, widely influential, executives scrambled, the industry was left reeling, it is worth noting, in a significant development, highlighting the importance of, underscoring concerns, this raises questions, this serves as a reminder, reassessing strategies, reimagining the future, poised, crucial, noteworthy, lasting impact, will enable, will play a key role, expected to have, significant milestone, important development, major implications, bold move, closely watched by the industry, adding to the chaos, amid ongoing uncertainty, amid growing concerns, the industry is watching, observers are watching, analysts expect, this could have far-reaching, a move that signals, a sign of things to come, hangs in the balance, significant setback, closely watched, will be watching closely, the stakes are high, remains to be seen, only time will tell, the world is watching, a watershed moment, a turning point, this underscores, this highlights, unique display, aerial prowess, state-of-the-art, cutting-edge, world-class, commitment to excellence, commitment to providing, ambitious growth strategy, crucial role in enhancing.
- TRUST THE FACTS: If the previous sentence already proved something is significant, the next sentence cannot announce that it is significant. The proof replaces the announcement. A €100 billion program collapsing is already important — you do not need to write "the future of European defence hangs in the balance." The collapse already proved it. Strong facts do not need commentary. Report them and move on.
- NO SPEC-PADDING: Do not fill space with aircraft specifications, fuel efficiency stats, cabin features, or corporate ambitions unless they are directly relevant to why the story happened. If the story is a formation flight, the reader does not need to know the 787 has composite materials. If the story is a range record, fuel efficiency is relevant. If the story is a delivery ceremony, it is not. Ask: does this spec explain why the story matters? If not, cut it.
