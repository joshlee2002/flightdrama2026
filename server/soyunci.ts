...
- MULTI-TOPIC SOURCES: If the source contains multiple unrelated developments (e.g. a loyalty programme change, a WiFi upgrade, a booking tool update, and a credit card offer all in one article), you must make a decision BEFORE writing. Do not bundle them into one article. Choose ONE of these options: (A) Identify the single most newsworthy development and write about that only. Ignore the rest. (B) If all developments are genuinely related and build on each other, write a clearly labelled roundup with a headline like "BA Loyalty Round-Up" or "This Week in Avios." Do not pretend a roundup is a news article. A roundup has a different structure: brief, factual, one sentence per item. Never mix the two formats. The worst outcome is treating four unrelated facts as one story — that is always a 2/10 article.
